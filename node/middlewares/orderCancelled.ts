...
export async function orderCancelled(
  ...
  if (ctx.body.domain === 'Marketplace') {
    ...
    if (
      settings.allowCancelledOrderSMS &&
      settings.orderCancelledText &&
      isMessageValid(settings.orderCancelledText)
    ) {
      const order = await oms.order(ctx.body.orderId)

      if (order != null) {
        const message = formatMessage(settings.orderCancelledText, order)
    ...
    } else if (settings.allowCancelledOrderSMS) {
